...
type AsanaEvent = 'deleted' | 'undeleted' | 'added' | 'deleted' | 'removed' | 'changed' | '*'
  ...
  async onStart(): Promise<void> {
    ...
    if (events.length) {
      if(!this.configOptions.workspaceId) {
        throw new Error('Asana Connector - error creating event, you must provide a workspaceId in your connector configuration')
      }
      ...
      const webhooks = await this._sdk.webhooks.getAll(this.configOptions.workspaceId) as AsanaWebhooks

      for (const {options: {gid, asanaEvent}} of events) {
        const existingWebhook = webhooks.data.find((asanaWebhook) => gid === asanaWebhook.resource.gid && asanaWebhook.target === webhookUrl && asanaWebhook.active)

        if(!existingWebhook) {
          const webhook = await this._sdk.webhooks.create(
              events[0].options.gid,
              webhookUrl,
              {},
          )
